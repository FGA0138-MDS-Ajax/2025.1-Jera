...
@router.get(
    "/movimentacao/estoque_atual/{id_produto}",
    dependencies=[Depends(require_profile(PerfilEnum.OPERADOR))],
)
...
@router.get("/movimentacao", status_code=200, dependencies=[Depends(require_profile(PerfilEnum.ADMINISTRADOR))])
...
@router.get(
    "/movimentacao/produto/{id_produto}",
    status_code=200,
    dependencies=[Depends(require_profile(PerfilEnum.ADMINISTRADOR))],
)
