...
from app.routers.schemas.lote import LoteCreateSchema, LoteResponseSchema, LoteUpdateSchema
...
@router.post("/lote", response_model=LoteResponseSchema, dependencies=[Depends(require_profile(PerfilEnum.GERENTE))])
def criar_lote(request_body: LoteCreateSchema):
    lote = Lote(
        ...
        quantidade_atual=0,
        data_entrada=dt.datetime.now(tz=dt.UTC),
    )
    return LoteService.criar_lote(lote)


@router.get(
    "/lote", response_model=list[LoteResponseSchema], dependencies=[Depends(require_profile(PerfilEnum.GERENTE))]
)
def listar_lotes():
...
@router.get(
    "/lote/{id_lote}", response_model=LoteResponseSchema, dependencies=[Depends(require_profile(PerfilEnum.GERENTE))]
)
...
@router.delete("/lote/{id_lote}", status_code=204, dependencies=[Depends(require_profile(PerfilEnum.GERENTE))])
...
@router.get("/lote/{id_lote}/validade_estado", dependencies=[Depends(require_profile(PerfilEnum.GERENTE))])
...
@router.patch(
    "/lote/{id_lote}/estado_estetico",
    response_model=LoteResponseSchema,
    dependencies=[Depends(require_profile(PerfilEnum.GERENTE))],
)
...
@router.get("/lote/{id_lote}/estado_predominante", dependencies=[Depends(require_profile(PerfilEnum.GERENTE))])
